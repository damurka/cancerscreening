...
These functions directly call the KHIS server to download the data require the setting of KHIS credentials. See [setting the credentials](https://cancerscreening.damurka.com/articles/set-your-credentials.html) for more information.
The get analytics call is supported by three metadata functions that help make the data complete and these 
include the `get_organisation_units_metadata()` to show where the data is from and `get_data_elements_metadata()` to show what the data is about and group the data into various categories.

The cancer screening data that are being tracked breast cancer, cervical cancer,
