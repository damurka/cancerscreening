...
cancerscreening provides an R interface to [Kenya Health Information System (KHIS)](https://hiskenya.org) via the [DHIS 2 API](https://docs.dhis2.org/en/develop/using-the-api/dhis-core-version-master/introduction.html). The goal of `cancerscreening` is to provide a easy way to download cancer screening data from the KHIS using R.
...
```
___Note: This package has not yet been released on CRAN.___
...
* Most function begin with the prefix `get_` followed by the screening area 
  `cervical`, `breast`, or `colorectal`. Auto-completion is your friend
...
* cancerscreening is "pipe-friendly" and, infact re-xports `%>%` but does not 
  require its use.
...
[Get Started](https://cancerscreening.damurka.com/articles/cancerscreening.html) is a more extenive general introoduction to cancerscreening.
